...
public class UserList {
    ...
    public static void partition(String sortFeature, CatenableQueue<User> qUnsorted, int pivot, 
        ...
        if (sortFeature.equals("id")) {
            while (!qUnsorted.isEmpty()) {
                temp = qUnsorted.dequeue();
                int tempId = temp.getId();
                if (tempId < pivot) {
                    qLess.enqueue(temp);
                } else if (tempId == pivot) {
                    qEqual.enqueue(temp);
                } else {
                    qGreater.enqueue(temp);
                }
            }
        } else {
            while (!qUnsorted.isEmpty()) {
                temp = qUnsorted.dequeue();
                int tempPg = temp.getPagesPrinted();
                if (tempPg < pivot) {
                    qLess.enqueue(temp);
                } else if (tempPg == pivot) {
                    qEqual.enqueue(temp);
                } else {
                    qGreater.enqueue(temp);
                }
    ...
    **/
    public static void quickSort(String sortFeature, CatenableQueue<User> q){ 
        ...
        if (sortFeature.equals("id")) {
            pivot = q.nth(rand).getId();
        } else {
            pivot = q.nth(rand).getPagesPrinted();
        }
        CatenableQueue<User> one = new CatenableQueue<User>();
        CatenableQueue<User> two = new CatenableQueue<User>();
        CatenableQueue<User> three = new CatenableQueue<User>();
        partition(sortFeature, q, pivot, one, two, three);
        quickSort(sortFeature, one);
        quickSort(sortFeature, three);
        one.append(two);
        one.append(three);
        q.append(one);
    }
    ...
    **/
    public CatenableQueue<CatenableQueue<User>> makeQueueOfQueues(){
        CatenableQueue<CatenableQueue<User>> ret = new CatenableQueue<CatenableQueue<User>>();
    ...
    **/
    public static CatenableQueue<User> mergeTwoQueues(String sortFeature, CatenableQueue<User> q1, CatenableQueue<User> q2) {
        CatenableQueue<User> ret = new CatenableQueue<User>();
        if (sortFeature.equals("id")) {
    ...
    **/
    public void mergeSort(String sortFeature){
        CatenableQueue<CatenableQueue<User>> cat = makeQueueOfQueues();
        while (cat.size() > 1) {
            CatenableQueue<User> one = cat.dequeue();
            CatenableQueue<User> two = cat.dequeue();
            CatenableQueue<User> merged = mergeTwoQueues(sortFeature, one, two); 
            cat.enqueue(merged);           
        }
        userQueue = cat.dequeue();
    }
